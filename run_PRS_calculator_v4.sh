...
for chr in $chrlist; do
  for anc in $ancestries; do
    echo "python3 PRS_calculator_v4.py -copyprobs_file_imputed $copyprobs_directory_imputed/temp.$anc.$chr.master_all_copyprobsperlocus.txt.gz -idfile $idfile -output_files $output_files -variants $variants -chr $chr -anc $anc -bootstrap $bootstrap -reverse_cols $reverse_cols" >> PRS_calculator_commands_temp
  done
done
j=50
